...
# 상수 정의
POPULATION_SIZE = 300  # 인구 크기 증가로 탐색 범위 확대
NUM_GENERATIONS = 150  # 세대 수 증가로 최적화 강화
...
def get_dynamic_param_ranges(volatility):
    ...
    if volatility > 30:  # 높은 변동성 환경
        return [
            ...
            (2.0, 5.0), (1.0, 3.0), (1.0, 3.0),  # w_strong_buy, w_weak_buy, w_sell
            (10, 30), (80, 95),  # stochastic_buy, stochastic_sell
            (0.5, 3.0), (0.5, 3.0),  # obv_weight, bb_width_weight
...
def calculate_fitness(portfolio_values):
    """피트니스 스코어 계산 (Sharpe Ratio, Calmar Ratio, Total Return 가중합)"""
    ...
    # Total Return에 더 높은 가중치 부여로 수익률 중시
    return 0.3 * sharpe_ratio + 0.3 * calmar_ratio + 0.4 * total_return
...
def cross_validation_evaluation(individual, folds=10):  # 교차 검증 fold 수 증가
...
def setup_toolbox(param_ranges):
    ...
    toolbox.register("mutate", tools.mutGaussian, mu=0, sigma=0.2, indpb=0.4)  # 변이 확률 증가
    toolbox.register("select", tools.selTournament, tournsize=5)  # 선택 압력 강화

def save_best_params(best_params):
    """최적 파라미터 저장"""
    data_to_save = {"version": "2.2", "parameters": best_params}
...
def main():
    ...
    for gen in tqdm(range(NUM_GENERATIONS), desc="Genetic Algorithm Progress"):
        offspring = algorithms.varAnd(population, toolbox, cxpb=0.8, mutpb=0.5)  # 변이 확률 증가
    ...
    print("Maximum Fitness (Weighted Sharpe, Calmar, and Total Return):", evaluate(best_ind)[0])
